...
Amazon search volume as an MCP tool. Plug into Claude, Cursor, or any MCP-compatible AI host. Weekly series, growth percentages, and live Amazon best sellers.
...
### pytrends alternative for Amazon data
...
series = client.get_trends(source=SOURCE, keyword="air fryer")
print(series[0])
# TrendsDataPoint(date='2026-03-28', value=72, keyword='air fryer', source='amazon')
...
```python
import asyncio
from amazon_trends_mcp import AsyncTrendsMcpClient, SOURCE

async def main():
    client = AsyncTrendsMcpClient(api_key="YOUR_API_KEY")
    series = await client.get_trends(source=SOURCE, keyword="air fryer")
    print(series[0])
...
google, youtube, reddit, amazon, tiktok = await asyncio.gather(
    client.get_trends(source="google search", keyword="air fryer"),
    client.get_trends(source="youtube",       keyword="air fryer"),
    client.get_trends(source="reddit",        keyword="air fryer"),
    client.get_trends(source="amazon",        keyword="air fryer"),
    client.get_trends(source="tiktok",        keyword="air fryer"),
)
```
...
```python
from amazon_trends_mcp import TrendsMcpClient, TrendsMcpError, SOURCE
...
**Do I need a Amazon developer account, OAuth token, or platform API key?**
No. One trendsmcp API key gives you access to all 30+ data sources.

**Will it break when Amazon changes its backend?**
